...
export default function CommentSection({ lpId }: Props) {
  ...
  const commentsKey = ['comments', lpId, sort] as const
  ...
  return (
    <section className="mt-8 border-t border-gray-700 pt-6">
        ...
        <div className="inline-flex overflow-hidden rounded border border-gray-600 text-xs font-medium">
          <button
            ...
            className={`px-3 py-1 transition-colors ${
              sort === 'asc' ? 'bg-white text-black' : 'bg-transparent text-gray-400 hover:text-white'
            }`}
          >
            오래된순
          </button>
          <button
            ...
            className={`border-l border-gray-600 px-3 py-1 transition-colors ${
              sort === 'desc' ? 'bg-white text-black' : 'bg-transparent text-gray-400 hover:text-white'
            }`}
          >
            최신순
          </button>
      ...
      {status === 'authenticated' && (
        <div className="mb-5 flex gap-2">
          <input
            ...
            className="flex-1 rounded-lg border border-gray-700 bg-gray-900 px-3 py-2 text-sm text-white outline-none placeholder-gray-500 focus:border-pink-500"
          />
          <button
      ...
      <ul className="flex flex-col gap-4">
          ...
          return (
            <li key={comment.id} className="flex items-start gap-3">
              ...
              {comment.author.avatar ? (
                ...
                <div className="grid h-8 w-8 shrink-0 place-items-center rounded-full bg-emerald-100 text-xs font-bold text-emerald-600">
                  ...
                  {isOwn && !isEditing && (
                    <div className="relative shrink-0">
                      ...
                      {openMenuId === comment.id && (
                        <div className="absolute right-0 top-7 z-10 flex gap-1 rounded-lg border border-gray-700 bg-[#1e1e2e] p-1 shadow-lg">
                          <button
                            onClick={() => startEdit(comment.id, comment.content)}
                            className="flex h-7 w-7 items-center justify-center rounded hover:bg-gray-700 text-gray-300 hover:text-white"
                            title="수정"
                          >
                          ...
                          <button
                            ...
                            className="flex h-7 w-7 items-center justify-center rounded hover:bg-gray-700 text-gray-300 hover:text-red-400"
                            title="삭제"
                          >
                            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                ...
                {isEditing ? (
                  <div className="flex items-center gap-2">
                    <input
                      ...
                      className="flex-1 rounded border border-pink-500 bg-gray-900 px-2 py-1 text-sm text-white outline-none"
                    />
                    <button
        ...
        {comments.length === 0 && (
          <li className="py-4 text-center text-sm text-gray-500">댓글이 없습니다.</li>
        )}
      </ul>
    </section>
  )
