...
function useThrottle<T>(value: T, delay: number = 3000): T {

    //1.상태 변수: throttledValue: 최종적으로 쓰로틀링 적용된 값
    //초기값을 전달받은 value
    const [throttledValue, setThrottledValue] = useState<T>(value);

    //2. ReflastExcecuted : 마지막으로 실행된 시간을 기록하는 변수
    ...
    useEffect(() => {
        ...
        if (Date.now() >= lastExcecuted.current + delay) {
            ...
            const timeId = setTimeout(() => {
                //타임가 만료되면, 마지막 업데이트 시간을 현재 시각으로 갱신합니다,
