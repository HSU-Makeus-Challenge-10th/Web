...
const Sidebar = ({ isOpen, onClose }: SidebarProps) => {
  ...
  return (
    <>
      ...
      <div
        onClick={onClose}
        className={`fixed inset-0 bg-black/50 z-20 backdrop-blur-sm transition-opacity duration-300 ${isOpen ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"
          }`}
      />
      ...
      <aside
        ref={sidebarRef}
        className={`fixed top-0 left-0 h-full w-64 bg-white dark:bg-gray-900 shadow-2xl z-30 transform transition-transform duration-300 ease-in-out ${isOpen ? "translate-x-0" : "-translate-x-full"
          }`}
      >
        <div className="flex flex-col h-full">
          ...
          <div className="flex items-center justify-between p-6 border-b border-gray-100 dark:border-gray-800">
            ...
            <button
              onClick={onClose}
              className="p-2 rounded-lg hover:bg-purple-50 dark:hover:bg-purple-950/40 transition-colors"
              aria-label="사이드바 닫기"
            >
              ...
              .map((item) => (
                <Link
                  ...
                  className={`flex items-center space-x-3 px-4 py-3 rounded-xl transition-all duration-200 ${isActive(item.path)
                    ...
                    : "text-gray-600 dark:text-gray-400 hover:bg-purple-50 dark:hover:bg-purple-950/40"
                    }`}
                >
            ...
            {accessToken && (
              <div className="pt-4 space-y-2">
                <button
                  ...
                  className="w-full flex items-center space-x-3 px-4 py-3 rounded-xl text-gray-600 dark:text-gray-400 hover:bg-purple-50 dark:hover:bg-purple-950/40 transition-all duration-200"
                >
          ...
          <div className="p-6 border-t border-gray-100 dark:border-gray-800">
            <p className="text-xs text-gray-400 text-center">
              © 2026 LP Project. All rights reserved.
            </p>
          </div>
        </div>
      </aside>
