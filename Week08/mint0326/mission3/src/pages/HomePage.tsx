...
import { useLpList } from '../hooks/lp/useLpList';
import { useSearch } from '../hooks/lp/useSearch';
...
const HomePage = () => {
    ...
    // Default list query
    const listQuery = useLpList(sort);
    
    // Search query with debounce
    const searchQueryResult = useSearch(debouncedQuery);
    ...
    }, [inView, throttledFetch, isFetchingNextPage]);
            ...
            {debouncedQuery && (
                <div className="mb-6 px-4 md:px-0">
                    <p className="text-xl text-gray-300">
                        "<span className="text-[#ff007f] font-bold">{debouncedQuery}</span>" 검색 결과
                    </p>
                </div>
            )}
            ...
            {!isLoading && allLps.length === 0 && (
                <div className="text-center py-32 border-2 border-dashed border-[#1a1a1a] rounded-3xl">
                    <p className="text-gray-500 text-xl font-medium">
                        {searchQuery ? '검색 결과가 없습니다.' : '컬렉션에 등록된 LP가 없습니다.'}
                    </p>
                </div>
            )}
        </div>
